...
def generate_typing_df(typing_set):
    ...
    for typing_file_list in typing_set:
        temp_array = np.array([[i for i in range(1, 28)]])
        for typing_file in typing_file_list:
            direction_group_df = read_tappy(typing_file)
...
def main():
    ...
    s = cross_val_score(clf, X_train, y_train, cv=5, scoring='accuracy')
    print(s.mean())
